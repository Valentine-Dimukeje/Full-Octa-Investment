...
const AdminWithdrawals = () => {
    ...
    const fetchWithdrawals = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setTransactions(data.filter(t => t.type === 'withdraw'));
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const handleRefresh = () => {
            fetchWithdrawals();
            toast.success("Withdrawals updated", { id: 'admin-refresh-toast' });
        };
    ...
    const processAction = async (id, action) => {
        ...
        const loadingToast = toast.loading(`${isDelete ? 'Deleting' : action === 'approve' ? 'Approving' : 'Rejecting'} withdrawal...`);
        ...
        try {
            ...
            if (res.ok) {
                toast.success(`Withdrawal ${isDelete ? 'deleted' : action + 'd'} successfully`, { id: loadingToast });
                fetchWithdrawals();
            } else {
    ...
    if (loading) return <AdminLayout title="Withdrawal Management"><div>Loading...</div></AdminLayout>;
    ...
    return (
        <AdminLayout title="Withdrawal Management">
            ...
            <ConfirmationModal 
                ...
                title={`${confirmModal.action === 'delete' ? 'Delete' : confirmModal.action === 'approve' ? 'Approve' : 'Reject'} Withdrawal`}
                message={`Are you sure you want to ${confirmModal.action} withdrawal request #${confirmModal.id}? ${confirmModal.action === 'delete' ? 'This action is irreversible.' : 'This will update the user\'s wallet balance.'}`}
